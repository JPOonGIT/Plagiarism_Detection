...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear23 = nn.Linear(zwischenlayer2, 1)
    ...
    def train_x2(self, features, labels, loss_function, optimizer, epochs):
        ...
        for epoch in range(epochs):
            for i, data in enumerate(features):
                data = Data_Preprocessing.preprocessing(data)
                optimizer.zero_grad()
                output2 = self.forward(data)
                loss_x2 = loss_function(output2, torch.tensor(labels[i]['multi-author']))
                print(' ist=', output2, ' soll=', torch.tensor(labels[i]['multi-author']), ' Loss=', loss_x2)
